...
class Asset(AccountsController):
	...
	def generate_qr_code(self):
		...
		if self.docstatus == 1:
			...
			url = url+"/check-status?asset_code="+str(self.name)
			qr.add_data(url)
...
def make_sales_invoice(asset, item_code, company, serial_no=None, cost_center=None, branch=None):
	...
	loss_disposal_account, gain_disposal_account, depreciation_cost_center = frappe.get_cached_value(
		"Company", company, ["disposal_account", "gain_disposal_account", "depreciation_cost_center"]
 	)
	si.append(
		"items",
		{
...
@frappe.whitelist()
def get_account_info(asset_category,company):
	asset_category_doc = frappe.get_doc("Asset Category", asset_category)
	asset_cat = []
	...
	if not asset_cat:
		...
		link = get_link_to_form("Asset Category", asset_category)
		frappe.throw(f"Set accounts setting for {company} in {link} asset category.")
