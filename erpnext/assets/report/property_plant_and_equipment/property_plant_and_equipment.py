...
def get_accounts(filters):
	data = []
	for a in frappe.db.sql("""SELECT ac.name,aca.fixed_asset_account AS fa,aca.accumulated_depreciation_account  AS acc,
				aca.depreciation_expense_account AS dep
				FROM `tabAsset Category` ac
				JOIN `tabAsset Category Account` aca
					ON aca.parent = ac.name
				GROUP BY ac.name
				""",
				as_dict=True,
		):
