...
class HostelChangeApplication(Document):
	...
	def validate(self):
		...
		if not self.hostel_check_in_form:
			frappe.throw(str("Before Hostel Change Application make a hostel check-in form"))
...
def get_hostel_checkin_form(applied_by, fiscal_year):
	result = frappe.db.sql("""
		...
		INNER JOIN `tabCheck-In Asset Items` cisi
			ON cisi.parent = hci.name
		WHERE cisi.student = %s
