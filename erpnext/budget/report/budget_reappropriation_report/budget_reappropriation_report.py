...
def get_data(filters):
    query = """
        select 
            ...
            t1.remark,
            t2.posting_date as date,
            (select account_currency from `tabAccount` where name = t2.to_account) as to_acc_currency,
            (select account_currency from `tabAccount` where name = t2.from_account) as from_acc_currency
        from 
            `tabBudget Reappropiation` as t1,
            `tabReappropriation Details` as t2
        where t2.reference = t1.name 
    ...
    if filters.to_cc:
        query += " and t2.to_cost_center = '{}'".format(filters.to_cc)

    if filters.from_cc:
        query += " and t2.from_cost_center = '{}'".format(filters.from_cc)

    if filters.to_acc:
        query += " and t2.to_account = '{}'".format(filters.to_acc)

    if filters.from_acc:
        query += " and t2.from_account = '{}'".format(filters.from_acc)
    ...
    if app_data:
        for a in app_data:
            row = {
                "to_cc": a.to_cc,
                "to_acc": a.to_acc,
                "to_acc_currency": a.to_acc_currency,
                "from_cc": a.from_cc,
                "from_acc": a.from_acc,
                "from_acc_currency": a.from_acc_currency,
                "amount": a.amount,
                "date": a.date,
                "remark": a.remark,
            }
