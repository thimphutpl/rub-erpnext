...
def get_data(filters):
	query = """
		SELECT bw.name, bw.cost_center, bw.budget_activity_type, bw.budget_activity, bw.budget_type, bw.withdrawal_amount,
		bw.remarks, bw.posting_date as date,
		CASE
			WHEN bw.budget_activity_type = 'Planning Activities'
				THEN pa.activities
			WHEN bw.budget_activity_type = 'Additional Activities'
				THEN aa.activities
		END AS budget_activity_name
		FROM `tabBudget Withdrawal` bw
		LEFT JOIN `tabPlanning Activities` pa
			ON pa.name = bw.budget_activity
		LEFT JOIN `tabAdditional Activities` aa
			ON aa.name = bw.budget_activity
		where bw.posting_date between '{from_date}' and '{to_date}' AND bw.college = '{college}'
		""".format(from_date=filters.from_date, to_date=filters.to_date, college=filters.college)

	# frappe.throw(str(query))
	if filters.cost_center:
		query+=" and bw.cost_center = \'" + filters.cost_center  + "\'"
	if filters.activity_type:
		query+=" and bw.budget_activity_type = \'" + filters.activity_type  + "\'"
	if filters.activity:
		query+=" and bw.budget_activity = \'" + filters.activity  + "\'"
