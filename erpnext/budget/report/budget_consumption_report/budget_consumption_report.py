    ...
    for d in datas:
        if filters.monthly_budget:
            ...
            supplement = flt(frappe.db.sql("""
                ...
                and cost_center="{cost_center}"
                and posting_date between '{from_date}' and '{to_date}'
            """.format(month=filters.month, from_date=from_date, to_date=to_date, 
                      account=d.account, cost_center=d.cost_center))[0][0], 2)
            ...
            monthly_received = frappe.db.sql("""
                ...
                and to_cost_center="{cost_center}"
            """.format(month=filters.month, from_date=from_date, to_date=to_date, 
                      account=d.account, cost_center=d.cost_center))[0][0]
        ...
        if filters.monthly_budget:
            ...
            """, (d.cost_center, d.account, from_date, to_date))[0][0]
            ...
            committed = frappe.db.sql("""
                ...
                where account = %s 
                and reference_date BETWEEN %s and %s
            """, (d.account, filters.from_date, filters.to_date))[0][0]
            ...
            committed = frappe.db.sql("""
                select SUM(amount) 
                from `tabCommitted Budget` 
                where cost_center = %s and project = %s 
                and reference_date BETWEEN %s and %s
            """, (d.cost_center, d.project, filters.from_date, filters.to_date))[0][0]
            ...
            committed = frappe.db.sql("""
                ...
                where cost_center = %s and account = %s 
                and reference_date BETWEEN %s and %s
            """, (d.cost_center, d.account, filters.from_date, filters.to_date))[0][0]
...
def construct_query(from_date, to_date, filters=None):
    ...
    if filters.monthly_budget and filters.month:
        ...
        query = """select b.cost_center, ba.account, b.project,
            (select a.account_number from `tabAccount` a where a.name = ba.account) as account_number,
            (select a.account_currency from `tabAccount` a where a.name = ba.account) as account_currency,
            ba.budget_type,
            SUM(ba.budget_amount) as budget_amount,
            sum(ba.{month_name}) as monthly_budget,
            SUM(ba.initial_budget) as initial_budget, 
