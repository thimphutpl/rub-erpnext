...
# 		entries = get_tds_invoices(self.company, self.from_date, self.to_date, \
...
# 		gl_entries   = []
# 		tds_account  = get_tds_account(self.company)
...
def get_tds_invoices(company, from_date, to_date, name, filter_existing=False, cond='', party_type=None, cost_center=None):
	...
	if not party_type or party_type == "Supplier":
		pi_entries = frappe.db.sql("""select t.posting_date, 'Purchase Invoice' as invoice_type, t.name as invoice_no,  
			...
			where t.posting_date between '{from_date}' and '{to_date}'
			and t.docstatus = 1
			and t1.add_deduct_tax = 'Deduct'
			{cost_center_cond}
			{existing_cond}
			{cond}""".format(
				cond=cond, 
				cost_center_cond=cost_center_cond,
	...
	je_entries = frappe.db.sql("""select t.posting_date, t.name as invoice_no, 'Journal Entry' as invoice_type,
		...
		{party_cond}
		{cond}""".format(
			je_cost_center_cond=je_cost_center_cond,
			cond=cond, 
			existing_cond=existing_cond,
			party_cond=party_cond, 
			from_date=from_date, 
