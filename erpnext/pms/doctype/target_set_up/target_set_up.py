...
class TargetSetUp(Document):
	...
	def on_update_after_submit(self):
		self.check_target()
		# if frappe.db.exists('Change In Performance Evaluation', {'employee': self.employee, 'fiscal_year':self.pms_calendar, 'docstatus': ('!=', 2)}):
		# 	return
		# elif frappe.db.exists('Review', {'target':self.name, 'pms_calendar': self.pms_calendar, 'docstatus':('!=',2)}):
		# 	frappe.throw("This target has a review. Changes not allowed.")
		...
		# 	frappe.throw("You cannot update target once it is evaluated")
	...
	def validate_calendar(self):
		# if frappe.db.exists("Change In Performance Evaluation", {"employee": self.employee, 
		# 	"fiscal_year": self.pms_calendar, "docstatus": ("!=", 2)}):
		# 	return

		# check for cancelled target and allow the target to re-evaluated   
	...
	def check_target(self):  
		if self.pms_group == "Group 3":
			frappe.throw(
					title='Error',
					msg="Employee under PMS Group 3 are not required to set Target")
	...
	def calculate_total_weightage(self):
		total = 0
		for t in self.target_item :
			total += flt(t.weightage)
		for item in self.common_target:
