...
class CaseTrackingSystem(Document):
	...
	if TYPE_CHECKING:
		...
		case_type: DF.Literal["", "Civil Litigation", "Criminal & ACC Cases", "HR Cases", "Disciplinary Cases", "Others"]
		...
		date: DF.Date
		date_of_receipt_of_referral: DF.Date | None
		...
		referral_document: DF.Attach
		referring_agency: DF.Link | None
		sanction_amount: DF.Float
		sanction_date: DF.Date | None
		status: DF.Literal["Draft", "Under Review", "Returned", "On-going", "Appeal", "Closed"]
	# end: auto-generated types

	def validate(self):
		for a in self.case_status:
			self.status = a.case_status
	def on_submit(self):
		pass
		# self.notify_users()
	...
	def autoname(self):
		type = ""
		if self.case_type == "HR Cases":
			type = "CASE/HR/"
		elif self.case_type == "Civil Litigation":
			type = "CASE/CL/"
		elif self.case_type == "Disciplinary Cases":
			type = "CASE/DC/"
		elif self.case_type == "Others":
			type = "CASE/O/"
	...
	def notify_users(self):
		...
		if not template:
			frappe.msgprint(_("Please set default template for Case Tracking Templates in HR settings."))
			return
